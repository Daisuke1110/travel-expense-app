...
- トリップ管理
  ...
  - トリップ作成
  - トリップ詳細表示
  - トリップ更新（タイトル/期間/レート、オーナーのみ。基準通貨は作成後は変更不可）
  - トリップ削除（オーナーのみ）
...
### 2.4 通貨・レート方針
- トリップごとに基準通貨を設定できる。
- MVP では基準通貨は作成後は変更不可（変更要求は 400）。レートのみ変更可。
- `amount` は通貨の最小単位の整数（小数なし）。フロントは小数入力を受け付けないか、受けた場合はバリデーションエラーを表示する（自動丸めはしない）。
...
### 2.9 データ更新方針（MVP）
- DynamoDB は PK/SK 更新不可のため、支出の `datetime` 変更は「GSI1 取得 → 新 SK で Put → 旧 Delete」で再書き込みする。
- トリップ削除は Trips/TripMembers/Expenses を順次削除し、部分失敗時はエラーとして再実行可能にする（将来ソフトデリート検討）。
- owner_name は Users に存在しない場合 `"owner"` を返す。
- トリップ日付（start/end）はトリップ現地日付として扱う。`datetime` は UTC 固定。
