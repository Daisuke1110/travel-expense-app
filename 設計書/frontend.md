...
| 設定（レート編集など） | `/settings` または詳細内セクション | レート編集 |
...
- 操作: 「支出を追加」ボタンでモーダル表示
- 権限表示: レート編集やトリップ編集操作は owner のみ、支出追加・削除は member/owner で表示

## 3.3 支出追加モーダル
  - 入力項目: 日付・時刻(既定=現在, UTC 保存), 金額(number), 通貨(既定=trip.base_currency), カテゴリ(food/transport/hotel/other), メモ(任意)
- バリデーション: 金額は 0 より大きい整数（通貨の最小単位）、日付/カテゴリは必須
...
- 機能: rate_to_jpy と base_currency を編集（owner のみ）
- 保存先 API: `PATCH /trips/{tripId}`（MVP 追加）
- レート変更後も過去支出はそのまま。表示時に最新レートで換算する旨を UI 文言で示す
