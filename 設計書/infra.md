...
- **ACM（SSL 証明書）**
  - 役割：
    - `travel.daisuke-selfstudy.com` 用の証明書。
  - ポイント：
...
- **API Gateway HTTP API**
  - 役割：
    - `https://travel-api.daisuke-selfstudy.com` で FastAPI（Lambda）を公開する。
  - ポイント：
...
- **Route 53**
  ...
  - レコード例：
    - `travel.daisuke-selfstudy.com` → CloudFront ディストリビューション（A レコード + ALIAS）
    - `travel-api.daisuke-selfstudy.com` → API Gateway カスタムドメイン

---

## 4.4 将来導入予定

※ infra には書いておくが、MVP では実際には作らない場合もある。
